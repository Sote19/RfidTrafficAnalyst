...
-- 📌 Insertar antenas por defecto para RFIDTrafficAnalyst
INSERT INTO antenas (id_empresa, ubicacion)
VALUES
  ((SELECT id_empresa FROM empresas WHERE nombre_empresa = 'RFIDTrafficAnalyst' LIMIT 1), 'ArduinoMega'),
  ((SELECT id_empresa FROM empresas WHERE nombre_empresa = 'RFIDTrafficAnalyst' LIMIT 1), 'ArduinoEsp');
...
CREATE TABLE IF NOT EXISTS rfid_senales (
    ...
    tiempo_entrada DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE USER 'admin'@'%' IDENTIFIED BY 'Mangorfid';
